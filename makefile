...
$(OUTPUT): $(SOURCES) 
	$(CC) -Isrc -I/usr/local/include/ $(SOURCES) $(USE_IMGUI) $(USE_VORBIS) $(USE_LIBPNG) $(USE_WEBGL) $(USE_ENGINE_WEBCORE) --bind -std=c++11 $(LDFLAGS) -o $(OUTPUT) -s WASM=1 -s ALLOW_MEMORY_GROWTH=1 -s ASSERTIONS=1 --preload-file data
