...
void Simulation::doSkyStep(CityMap* a_map)
{
  ...
  for (int i = 0; i < a_map->m_width; ++i)
  {
    for (int j = 0; j < a_map->m_height; ++j)
    {
      ...
      if (tile.tileType == TILE_ROAD && tile.isDefault == 0) source += 15;
      if (tile.tileType == TILE_BUILD) source += 150;
      if (tile.tileType == TILE_TREE) source -= 15;
...
void Simulation::doSurfaceUtilities(CityMap* a_map, int resource)
{
  ...
  for (int i = 0; i < a_map->m_width; ++i)
  {
    for (int j = 0; j < a_map->m_height; ++j)
    {
      ...
      {
        // Water
        if (tile.pipeType == 2) engageRadius(a_map, i, j, 4, resource);
      }
      ...
      if (resource == 1)
      {
        ...
        if (tile.tileType == TILE_BUILD)
        {
          ...
          if (data.resourceType == resource && tile.utilityFlag & (1<<resource))
